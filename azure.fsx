...
let folders take path dirName (containerName: string) =
    ...
    |> defaultArg (Option.map Seq.take take) id
    |> Seq.map (fun dirPath ->
...
Azure.Containers.data.Upload(indexFile) |> Async.RunSynchronously
let index = Azure.Containers.data.``index.json``.Read()

// THESE CAN TAKE A LONG TIME TO RUN!
folders None (Path.Combine(indexPath, "source")) "source" "data"
folders None (Path.Combine(indexPath, "formatted")) "formatted" "data"
